...
def test_submit(username, live_server, webdriver):  # pylint: disable=W0621
    ...
    try:
        ...
        assert selenium.current_url == live_server.url + '/measurement/'
...
def __fill_in_single_measurement(selenium):
    ...
    file_name.send_keys(os.path.join(settings.BASE_DIR,
...
@pytest.mark.django_db
def test_measurement_file_dir(live_server):  # pylint: disable=W0613
    create_grouped_users()
    create_correct_sample_data()
