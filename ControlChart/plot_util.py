...
class PlotGenerator(object):
    ...
    def __fetch_plot_data(self, filter_args):
        vals = ControlChart.models.CharacteristicValue.objects.filter(
            _finished=True,
            **filter_args)
    ...
    def __save_user_session(self, document, index):
        with closing(push_session(document)) as session:
            ...
            _ = ControlChart.models.UserPlotSession.objects.create(
...
def update_plot_sessions():
    ...
    """
    for usession in ControlChart.models.UserPlotSession.objects.all():
        with closing(
